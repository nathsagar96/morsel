...
public class JwtTokenProvider {
    ...
    public String generateAccessToken(
            Long userId, String username, String email, String role, boolean enabled, boolean accountNonLocked) {
        Date now = new Date();
    ...
    public Optional<AccessTokenClaims> validateAccessTokenWithClaims(String token) {
        try {
            Claims claims = parseToken(token);
            if (!TOKEN_TYPE_ACCESS.equals(claims.get(CLAIM_TYPE, String.class))) {
                log.debug("Token is not an access token");
